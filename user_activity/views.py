...
class CommentListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        for profile in followings:
            following_profiles_posts = following_profiles_posts | profile.posts.filter(is_active=True)
        all_posts = user_posts | following_profiles_posts
        
        all_comments = Comment.objects.none()
        for post in all_posts:
            all_comments = all_comments | Comment.objects.filter(post=post, is_active=True)
        return all_comments
